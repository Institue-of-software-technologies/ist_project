            ...
            <div class="bg-white border border-lg border-gray-300 hover:border-gray-500 rounded-lg overflow-hidden mt-4 p-4">
                ...
                <div class="mt-4">
                    <a href="{{ asset('storage/' . $application->cv) }}"
                        class="bg-red-600 text-white text-center rounded-xl p-3 white" target="_blank"><i class="fas fa-download"></i> Download CV</a>
                </div>
                ...
                <div class="mt-4">
                    <form action="{{ route('applications.sendEmail', $application->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="bg-blue-500 text-white text-center rounded-xl p-3">Send Message</button>
                    </form>
                </div>
            </div>
