                ...
                @endif
                    <div class=" grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-6">
                        @foreach ($projects as $project)
                            <div class="bg-gray-200 shadow-md rounded-lg p-5 border border-gray-800">
                                ...
                                @if ($project->flowchart_diagram)
                                    <div class="shadow-2xl shadow-gray-700 rounded-xl p-1 mt-8">
                                        <p class="mt-2 text-xl lg:text-3xl text-gray-900"><strong>Flowchart
                                                Diagram:</strong></p>
                                        <img src="{{ asset('storage/' . $project->flowchart_diagram) }}"
                                            alt="Flowchart diagram" class="mt-2 w-full h-auto rounded-xl">
                                    </div>
                                @endif
                                @if ($project->database_diagram)
                                    <div class="shadow-2xl shadow-gray-700 rounded-xl p-1 mt-8">
                                        <p class="mt-2 text-2xl lg:text-4xl text-gray-900"><strong><i
                                                    class="fa-solid fa-database"></i> Diagram:</strong></p>
                                        <img src="{{ asset('storage/' . $project->database_diagram) }}"
                                            alt="Database diagram" class="mt-2 w-full h-auto rounded-xl">
                                    </div>
                                @endif

                                <div class="shadow-2xl shadow-gray-700 rounded-xl p-1 mt-8">
                                    <label class="block text-gray-700 text-2xl lg:text-4xl font-bold mb-2"
                                        for="video_url"><i class="fa-solid fa-video"></i></label>
                                    @if (isset($project) && $project->video_url)
                                        <video class="w-full h-auto mt-2 rounded-xl" autoplay muted>
                                            <source src="{{ asset('storage/' . $project->video_url) }}"
                                                type="video/mp4">
