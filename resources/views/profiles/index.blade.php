    ...
    <div class="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl w-full bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="profile-header bg-gradient-to-r from-red-500 to-red-700 py-6 text-center text-white">
                <a class="bg-red-500 hover:bg-red-700 text-white font-bold my-auto px-4 rounded float-right" href="{{ route('dashboard') }}">Back</a>
                <h1 class="text-4xl font-bold mb-2">{{ __('Alumni Profiles') }}</h1>
                <form action="{{ route('profiles.search') }}" method="GET" class="mt-4">
                    <input type="text" name="search" class="py-2 px-4 text-gray-900 font-semibold border rounded-lg w-1/2" placeholder="Search by name">
                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg">Search</button>
                </form>
            </div>
                                ...
                                <div class="p-4">
                                    @if ($profile->profile_photo)
                                        <div class="flex justify-center mb-4">
                                            <img src="{{ asset('storage/' . $profile->profile_photo) }}" alt="{{ $profile->full_name }}" class="w-32 h-32 rounded-full shadow-lg shadow-gray-300 border-4 border-gray-500">
                                        </div>
                                    @endif
                                    <p class="text-gray-700"><i class="far fa-envelope"></i> {{ $profile->email }}</p>
                                    <p class="text-gray-700"><i class="fas fa-map-marker-alt"></i> {{ $profile->location }}</p>
                                    <p class="text-gray-700"><i class="fas fa-graduation-cap"></i> {{ $profile->degree }}</p>
                                    <p class="text-gray-700"><i class="fas fa-briefcase"></i> {{ $profile->current_job_title }}</p>
