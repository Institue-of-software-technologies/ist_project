        ...
        <div class="card bg-white shadow-md rounded-md overflow-hidden">
          ...
          <div class="card-body px-4 py-5">
            ...
            <form action="{{ url('roles/'.$role->id.'/give-permissions') }}" method="POST">
                  ...
                  @foreach ($permissions as $permission)
                    <div class="flex items-center">
                      <input
                       type="checkbox"
                       name="permission[]" 
                       value="{{ $permission->name }}" {{ in_array($permission->id, $rolePermissions) ? 'checked' : '' }} class="w-4 h-4 mr-3 text-blue-600 focus:ring-blue-500 rounded-sm">
              ...
              <div class="mb-3">
                <button type="submit" class="text-white bg-blue-500 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 shadow-sm">Update</button>
              </div>
            </form>
          </div>
        </div>
